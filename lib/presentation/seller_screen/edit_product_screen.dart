...
class _EditProductScreenState extends State<EditProductScreen> {
  ...
  void initState() {
    super.initState();
    _productId = widget.productData['id'] ?? widget.productData['productId'] ?? 0;
    ...
    _brandController = TextEditingController(text: widget.productData['brandName'] ?? '');
  ...
  Widget _buildSubCategoryDropdown(CategoryProvider provider) {
    final subcategories = _selectedCategoryId == null || _useNewCategory
        ? []
        : provider.categories
            .firstWhere((c) => c.categoryId == _selectedCategoryId, orElse: () => provider.categories.first)
            .subCategories;
